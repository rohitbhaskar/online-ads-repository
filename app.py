...
def my_input_index_data(num_docs=None, batch_size=8, dataset='f30k'):
    from myDataset import get_data_loader
    captions = 'dataset_flickr30k.json' if dataset == 'f30k' else 'captions.txt'
    data_loader = get_data_loader(root=os.path.join(cur_dir, f'data/{dataset}/images'),
                                  captions=os.path.join(cur_dir, f'data/{dataset}/{captions}'),
                                  split='test',
...
@click.option('--data_set', '-d', type=click.Choice(['f30k', 'f8k', 'my-data'], case_sensitive=False), default='f30k')
def main(task, num_docs, batch_size, data_set):
    ...
    if task == 'index':
        ...
        f = Flow().load_config('flow-index.yml')
        with f:
            f.index(input_fn=my_input_index_data(num_docs, batch_size, data_set), batch_size=batch_size)
